...
export default function LoginPage() {
  return (
    <main className="min-h-screen flex items-center justify-center bg-[#f8fafc] dark:bg-[#070a12] p-4 sm:p-6 text-slate-900 dark:text-white relative overflow-hidden transition-colors duration-300">
      ...
      <Suspense fallback={
        <div className="relative w-full max-w-md rounded-3xl border border-slate-200 dark:border-white/15 bg-white dark:bg-slate-900/90 p-8 text-center">
          <div className="h-8 w-8 mx-auto border-4 border-amber-400 border-t-transparent rounded-full animate-spin mb-4" />
          <p className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Loading login page...</p>
        </div>
      }>
        <LoginForm />
      </Suspense>
